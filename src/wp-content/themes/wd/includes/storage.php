...
if ( ! function_exists( 'wd_storage_get' ) ) {
	/**
	 ...
	 */
	function wd_storage_get( $var_name, $default = '' ) {
		global $WD_STORAGE;
		return isset( $WD_STORAGE[ $var_name ] ) ? $WD_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'wd_storage_set' ) ) {
	/**
	 ...
	 */
	function wd_storage_set( $var_name, $value ) {
		global $WD_STORAGE;
		$WD_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'wd_storage_empty' ) ) {
	/**
	 ...
	 */
	function wd_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $WD_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $WD_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $WD_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $WD_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'wd_storage_isset' ) ) {
	/**
	 ...
	 */
	function wd_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $WD_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $WD_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $WD_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $WD_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'wd_storage_unset' ) ) {
	/**
	 ...
	 */
	function wd_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $WD_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $WD_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $WD_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $WD_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'wd_storage_inc' ) ) {
	/**
	 ...
	 */
	function wd_storage_inc( $var_name, $value = 1 ) {
		global $WD_STORAGE;
		if ( empty( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = 0;
		}
		$WD_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'wd_storage_concat' ) ) {
	/**
	 ...
	 */
	function wd_storage_concat( $var_name, $value ) {
		global $WD_STORAGE;
		if ( empty( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = '';
		}
...
if ( ! function_exists( 'wd_storage_get_array' ) ) {
	/**
	 ...
	 */
	function wd_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $WD_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $WD_STORAGE[ $var_name ][ $key ] ) ? $WD_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $WD_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $WD_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'wd_storage_set_array' ) ) {
	/**
	 ...
	 */
	function wd_storage_set_array( $var_name, $key, $value ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$WD_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'wd_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function wd_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $WD_STORAGE[ $var_name ][ $key ] ) ) {
			$WD_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$WD_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$WD_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'wd_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function wd_storage_merge_array( $var_name, $key, $value ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$WD_STORAGE[ $var_name ] = array_merge( $WD_STORAGE[ $var_name ], $value );
		} else {
			$WD_STORAGE[ $var_name ][ $key ] = array_merge( $WD_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'wd_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function wd_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			wd_array_insert_after( $WD_STORAGE[ $var_name ], $after, $key );
		} else {
			wd_array_insert_after( $WD_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'wd_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function wd_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			wd_array_insert_before( $WD_STORAGE[ $var_name ], $before, $key );
		} else {
			wd_array_insert_before( $WD_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'wd_storage_push_array' ) ) {
	/**
	 ...
	 */
	function wd_storage_push_array( $var_name, $key, $value ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $WD_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $WD_STORAGE[ $var_name ][ $key ] ) ) {
				$WD_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $WD_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'wd_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function wd_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $WD_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $WD_STORAGE[ $var_name ] ) && is_array( $WD_STORAGE[ $var_name ] ) && count( $WD_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $WD_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $WD_STORAGE[ $var_name ][ $key ] ) && is_array( $WD_STORAGE[ $var_name ][ $key ] ) && count( $WD_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $WD_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'wd_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function wd_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( empty( $WD_STORAGE[ $var_name ][ $key ] ) ) {
			$WD_STORAGE[ $var_name ][ $key ] = 0;
		}
		$WD_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'wd_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function wd_storage_concat_array( $var_name, $key, $value ) {
		global $WD_STORAGE;
		if ( ! isset( $WD_STORAGE[ $var_name ] ) ) {
			$WD_STORAGE[ $var_name ] = array();
		}
		if ( empty( $WD_STORAGE[ $var_name ][ $key ] ) ) {
			$WD_STORAGE[ $var_name ][ $key ] = '';
		}
		$WD_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'wd_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function wd_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $WD_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $WD_STORAGE[ $var_name ] ) ? $WD_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $WD_STORAGE[ $var_name ] ) ? $WD_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'wd_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function wd_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $WD_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $WD_STORAGE[ $var_name ]->$prop ) ? $WD_STORAGE[ $var_name ]->$prop : $default;
	}
}
