...
if ( ! function_exists( 'wd_array_get_first' ) ) {
	/**
	 ...
	 */
	function wd_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'wd_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function wd_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'wd_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function wd_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'wd_array_from_list' ) ) {
	/**
	 ...
	 */
	function wd_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'wd_array_slice' ) ) {
	/**
	 ...
	 */
	function wd_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'wd_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * wd_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * wd_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
...
if ( ! function_exists( 'wd_array_insert_after' ) ) {
	/**
	 ...
	 */
	function wd_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = wd_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = wd_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'wd_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function wd_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'wd_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function wd_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'wd_generate_id' ) ) {
	/**
	 ...
	 */
	function wd_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'wd_get_tag' ) ) {
	/**
	 ...
	 */
	function wd_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'wd_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function wd_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'wd_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function wd_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( wd_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'wd_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function wd_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'wd_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function wd_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'wd_get_column_class' ) ) {
	/**
	 ...
	 */
	function wd_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'wd_stripslashes' ) ) {
	/**
...
if ( ! function_exists( 'wd_get_value_gp' ) ) {
	/**
	 ...
	 */
	function wd_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'wd_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function wd_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'wd_get_value_gps' ) ) {
	/**
	 ...
	 */
	function wd_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'wd_get_session_value' ) ) {
	/**
	 ...
	 */
	function wd_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'wd_set_session_value' ) ) {
	/**
	 ...
	 */
	function wd_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'wd_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function wd_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'wd_get_cookie' ) ) {
	/**
	 ...
	 */
	function wd_get_cookie( $name, $defa = '' ) {
		return wd_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'wd_set_cookie' ) ) {
	/**
	 ...
	 */
	function wd_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'wd_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//wd_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//wd_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'wd' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'wd_hex2rgba' ) ) {
	/**
...
if ( ! function_exists( 'wd_hex2hsb' ) ) {
	 ...
	 */
	function wd_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = wd_rgb2hsb( wd_hex2rgb( $hex ) );
...
if ( ! function_exists( 'wd_hsb2hex' ) ) {
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function wd_hsb2hex( $hsb ) {
		return wd_rgb2hex( wd_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'wd_date_to_sql' ) ) {
	/**
...
if ( ! function_exists( 'wd_format_price' ) ) {
	/**
	 ...
	 */
	function wd_format_price( $price ) {
...
if ( ! function_exists( 'wd_num2size' ) ) {
	/**
	 ...
	 */
	function wd_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'wd_size2num' ) ) {
	/**
	 ...
	 */
	function wd_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'wd_parse_num' ) ) {
	/**
...
if ( ! function_exists( 'wd_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function wd_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'wd_remove_macros' ) ) {
	/**
...
if ( ! function_exists( 'wd_is_inherit' ) ) {
	/**
...
if ( ! function_exists( 'wd_strshort' ) ) {
	 ...
	 */
	function wd_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = wd_strip_tags( $str );
...
if ( ! function_exists( 'wd_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function wd_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'wd_strip_tags' ) ) {
	/**
...
if ( ! function_exists( 'wd_excerpt' ) ) {
	 ...
	 */
	function wd_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return wd_strwords( wd_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'wd_unserialize' ) ) {
	/**
...
if ( ! function_exists( 'wd_str_replace' ) ) {
	/**
	 ...
	 */
	function wd_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = wd_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( wd_str_replace( $from, $to, wd_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'wd_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function wd_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'wd_tags_count' ) ) {
	/**
	 ...
	 */
	function wd_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'wd_getimagesize' ) ) {
	/**
	 ...
	 */
	function wd_getimagesize( $url, $echo = false ) {
		$img_size = false;
		$img_path = wd_is_url( $url ) ? wd_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'wd_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function wd_clear_thumb_size( $url, $remove_protocol = true ) {
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = wd_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'wd_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function wd_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = wd_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = wd_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = wd_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = wd_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'wd_get_thumb_size' ) ) {
	 ...
	 */
	function wd_get_thumb_size( $ts ) {
		$retina = wd_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'wd_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'wd_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'wd-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'wd_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'wd-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'wd_get_closest_thumb_size' ) ) {
	 ...
	 */
	function wd_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'wd_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function wd_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = wd_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'wd_get_post_image' ) ) {
	/**
	 ...
	 */
	function wd_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'wd_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function wd_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = wd_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = wd_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = wd_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( wd_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = wd_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'wd_get_post_audio_list' ) ) {
	/**
...
if ( ! function_exists( 'wd_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function wd_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = wd_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = wd_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'wd_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'wd_get_post_video_list' ) ) {
	/**
...
if ( ! function_exists( 'wd_get_post_iframe' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function wd_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = wd_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = wd_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'wd_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'wd_get_embed_video' ) ) {
	/**
	 ...
	 */
	function wd_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			// $embed_video = wd_make_video_autoplay( $embed_video );
		} else if ( wd_is_from_uploads( $video ) ) {
			...
			$video = wd_add_to_url( $video, wd_get_embed_video_parameters() );
			$tag   = wd_get_embed_video_tag_name();
			// Calc video width and height (with ratio 16:9)
			$ratio  = explode( ':', apply_filters( 'wd_filter_video_ratio', '16:9' ) );
			$width  = wd_get_content_width();
			$height = round( $width / $ratio[0] * $ratio[1] );
			$dim    = apply_filters( 'wd_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									) );
...
if ( ! function_exists( 'wd_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function wd_make_video_autoplay( $video, $muted = false ) {
...
if ( ! function_exists( 'wd_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function wd_is_from_uploads( $url ) {
		$url          = wd_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = wd_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'wd_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function wd_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'wd_is_vimeo_url' ) ) {
	/**
...
if ( ! function_exists( 'wd_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'wd_init_filesystem', 0 );
	/**
	 ...
	 */
	function wd_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'wd_fpc' ) ) {
	 ...
	 */
	function wd_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( wd_is_on( wd_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'wd' ), $file ) );
					}
				}
			}
		}
	...
	function wd_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? wd_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( wd_is_on( wd_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'wd' ), $file ) );
					}
				}
			}
		}
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( wd_is_on( wd_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'wd' ), $file ) );
					}
				}
			}
		}
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( wd_is_on( wd_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'wd' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( wd_is_on( wd_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'wd' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'wd_unlink' ) ) {
	 ...
	 * @throws Exception        Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function wd_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( wd_is_on( wd_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'wd' ), $path ) );
					}
				}
			}
		}
	...
	function wd_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = wd_prepare_path( $src );
				$dst = wd_prepare_path( $dst );
...
if ( ! function_exists( 'wd_unzip_file' ) ) {
	/**
...
if ( ! function_exists( 'wd_esc' ) ) {
	 ...
	 */
	function wd_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'wd_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'wd_check_min_file' ) ) {
	 ...
	 */
	function wd_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && wd_is_off( wd_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && wd_is_off( wd_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'wd_get_file_dir' ) ) {
	/**
	 ...
	 */
	function wd_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! WD_ALLOW_SKINS && ! wd_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'wd_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( WD_CHILD_DIR != WD_THEME_DIR && file_exists( WD_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? WD_CHILD_URL : WD_CHILD_DIR ) . wd_check_min_file( $file, WD_CHILD_DIR );
			} elseif ( file_exists( WD_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? WD_THEME_URL : WD_THEME_DIR ) . wd_check_min_file( $file, WD_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'wd_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function wd_get_file_url( $file ) {
		return wd_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'wd_get_file_ext' ) ) {
	/**
	 ...
	 */
	function wd_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'wd_get_file_name' ) ) {
	/**
	 ...
	 */
	function wd_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'wd_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function wd_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'wd_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( WD_CHILD_DIR != WD_THEME_DIR && is_dir( WD_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? WD_CHILD_URL : WD_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( WD_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? WD_THEME_URL : WD_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'wd_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'wd_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function wd_get_folder_url( $folder ) {
		return wd_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'wd_merge_js' ) ) {
	/**
	 ...
	 */
	function wd_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= wd_fgc( wd_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = wd_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = wd_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			wd_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'wd' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'wd_filter_js_output', apply_filters( 'wd_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'wd_merge_css' ) ) {
	/**
	 ...
	 */
	function wd_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'wd_filter_responsive_sizes', wd_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = wd_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = wd_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'wd_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'wd_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'wd' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'wd' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = wd_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			wd_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'wd') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'wd_filter_css_output', apply_filters( 'wd_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'wd_filter_merge_css' ) ) {
	add_filter( 'wd_filter_merge_css', 'wd_filter_merge_css' );
	/**
	 ...
	 */
	function wd_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'wd_merge_sass' ) ) {
	/**
	 ...
	 */
	function wd_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'wd_filter_responsive_sizes', wd_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'wd_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			wd_fpc( wd_get_file_dir( $to ), apply_filters( 'wd_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'wd_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function wd_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'wd_add_protocol' ) ) {
	/**
	 ...
	 */
	function wd_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? wd_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'wd_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function wd_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'wd_add_to_url' ) ) {
	/**
	 ...
	 */
	function wd_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = wd_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'wd_is_url' ) ) {
	 ...
	 */
	function wd_is_url( $url ) {
		return strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false;
	}
}

if ( ! function_exists( 'wd_is_external_url' ) ) {
	/**
	 ...
	 */
	function wd_is_external_url( $url ) {
		return wd_is_url( $url ) && strpos( $url, wd_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}

if ( ! function_exists( 'wd_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function wd_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => wd_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => WD_CHILD_DIR,
				'url' => wd_remove_protocol_from_url( WD_CHILD_URL )
				),
			'theme' => array(
				'dir' => WD_THEME_DIR,
				'url' => wd_remove_protocol_from_url( WD_THEME_URL )
				)
			);
