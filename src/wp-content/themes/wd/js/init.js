...
jQuery( document ).ready( function() {
	...
	function wd_init_media_elements(cont) {
		var audio_selector = wd_apply_filters( 'wd_filter_mediaelements_audio_selector', 'audio:not(.inited)' ),
			video_selector = wd_apply_filters( 'wd_filter_mediaelements_video_selector', 'video:not(.inited):not([nocontrols]):not([controls="0"]):not([controls="false"]):not([controls="no"])' ),	//:not([autoplay])
	...
	function wd_infinite_scroll_in_single() {
		...
		$single_post_scrollers.each( function() {
			...
			// Load next post
				...
				jQuery.get( wd_add_to_url( link, { 'action': 'prev_post_loading' } ) ).done(
					function( response ) {
						// Get inline styles and add to the page styles
						wd_import_inline_styles( response );
						// Get tags 'link' from response and add its to the 'head'
	...
	function wd_shift_under_panels() {
		...
		if ($body.hasClass( 'header_position_under' ) && ! wd_browser_is_mobile()) {
			...
			if ( $body.hasClass( 'mobile_layout' ) ) {	//wd_window_width() < WD_STORAGE['mobile_breakpoint_underpanels_off'] ) {
			...
			if (scroll_offset > 0) {
				var offset = scroll_offset;	// - wd_adminbar_height();
