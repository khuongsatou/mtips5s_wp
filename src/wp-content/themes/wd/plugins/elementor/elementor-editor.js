...
jQuery( document ).ready(
	function() {
		...
		wd_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="wd_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.wd_post_editor' );
						if ( link.length > 0 ) {
							wd_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.wd_post_editor', function(e) {
						wd_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('wd_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function wd_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
