...
if ( ! function_exists( 'wd_gutenberg_fse_replace_frontpage_content' ) ) {
	add_filter( 'frontpage_template', 'wd_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	/**
	 ...
	 * 
	 * Hooks: add_filter( 'frontpage_template', 'wd_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	 *
	 * @param string   $template  Path to the template. See locate_template().
	 * @param string   $type      Sanitized filename without extension.
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function wd_gutenberg_fse_replace_frontpage_content( $template, $type = '', $templates = array() ) {
...
if ( ! function_exists( 'wd_gutenberg_fse_replace_no_posts_content' ) ) {
	add_filter( 'search_template',  'wd_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'index_template',   'wd_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	add_filter( 'archive_template', 'wd_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	/**
	 * Substitute a content of the global variable $_wp_current_template_content
	 * with a content of the template 'parts/none-archive.html' or 'templates/none-archive.html'.
	 * 
	 * Hooks: add_filter( 'search_template',  'wd_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'index_template',   'wd_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 * 
	 *        add_filter( 'archive_template', 'wd_gutenberg_fse_replace_no_posts_content', 9999, 3 );
	 ...
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function wd_gutenberg_fse_replace_no_posts_content( $template, $type = '', $templates = array() ) {
		if ( substr( $template, -19 ) == 'template-canvas.php' ) {
			if ( ! have_posts() ) {
				...
				$part_exists = false;
				$data = wd_gutenberg_fse_theme_json_data();
				...
				$no_posts_file = wd_get_file_dir( ( $part_exists ? 'parts/' : 'templates/' ) . "{$part_name}.html" );
				if ( ! empty( $no_posts_file ) ) {
					// Check if exists a saved template with a same name
					$posts = get_posts( array(
						'name'           => $part_name,
						'posts_per_page' => 1,
						'post_type'      => defined( 'WD_FSE_TEMPLATE_PART_PT' ) ? WD_FSE_TEMPLATE_PART_PT : 'wp_template_part',
					) );
					// Get a template content from the database or from the file (if a saved version is not found)
					$content = ! empty( $posts[0]->post_content ) ? $posts[0]->post_content : wd_fgc( $no_posts_file );
...
if ( ! function_exists( 'wd_gutenberg_fse_show_template_as_layout' ) ) {
	add_filter( 'wd_filter_custom_layout_shown', 'wd_gutenberg_fse_show_template_as_layout', 10, 3 );
	/**
	 * Show an FSE template (header/footer/sidebar) inside a non-FSE templates.
	 * 
	 * Hook: add_filter( 'wd_filter_custom_layout_shown', 'wd_gutenberg_fse_show_template_as_layout', 10, 3 );
	 * 
	 ...
	 */
	function wd_gutenberg_fse_show_template_as_layout( $shown, $layout_id = '', $post_id = 0 ) {
		// If a layout_id is a string with a template part name
		if ( ! $shown && ! empty( $layout_id ) && is_string( $layout_id) && (int)$layout_id === 0 && wd_gutenberg_is_fse_theme() ) {
			// And file with template-name.html is present in the theme subfolder '/parts'
			$template_path = wd_get_file_dir( "/parts/{$layout_id}.html" );
			if ( ! empty( $template_path ) ) {
				$template_content = wd_fgc( $template_path );
				if ( ! empty( $template_content ) ) {
					wd_show_layout( wd_filter_post_content( $template_content ) );
					$shown = true;
				}
			}
		}
...
if ( ! function_exists( 'wd_gutenberg_fse_modify_template_with_current_options' ) ) {
	foreach( wd_get_wp_template_hooks() as $hook ) {
		add_filter( $hook, 'wd_gutenberg_fse_modify_template_with_current_options', 30, 3 );
	}
	/**
	 ...
	 * 
	 * Trigger the filter 'wd_filter_wp_current_template_content' to allow other modules
	 ...
	 */
	function wd_gutenberg_fse_modify_template_with_current_options( $template, $type = '', $templates = array() ) {
		global $_wp_current_template_content;
		if ( substr( $template, -19 ) == 'template-canvas.php' && is_array( $templates ) ) {
			$_wp_current_template_content = apply_filters( 'wd_filter_wp_current_template_content', $_wp_current_template_content, $template, $type, $templates );
		}
		return $template;
	}
}

if ( ! function_exists( 'wd_gutenberg_fse_modify_template_replace_macros' ) ) {
//	add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_macros', 100, 4 );
	/**
	 * Replace a macros {{Y}} and {Y} in the content of the current template to the current year.
	 * 
	 * Hooks: add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_macros', 10, 4 );
	 *
	 ...
	 */
	function wd_gutenberg_fse_modify_template_replace_macros( $content, $template, $type, $templates ) {
...
// Header
//----------------------------------------------
if ( ! function_exists( 'wd_gutenberg_fse_modify_template_replace_header' ) ) {
	add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_header', 10, 4 );
	/**
	 * Replace a header in the content of the current template
	 * according to the Theme Options of the current page.
	 * 
	 * Hooks: add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_header', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 ...
	 * @return string  A filtered content of the template.
	 */
	function wd_gutenberg_fse_modify_template_replace_header( $content, $template, $type, $templates ) {
		$header_type = wd_get_theme_option( 'header_type' );
		if ( 'custom' == $header_type ) {
			$header_style = wd_get_theme_option( "header_style" );
			...
			if ( strpos( $header_style, "header-fse-template-" ) !== false ) {
				...
				ob_start();
				do_action( 'wd_action_fse_before_custom_header', $content, $template, $type, $templates );
				...
				ob_start();
				do_action( 'wd_action_fse_after_custom_header', $content, $template, $type, $templates );
			...
			} else if ( wd_is_layouts_available() ) {
				ob_start();
				// Trigger action before the custom header to allow other modules include an additional content to the custom header
				do_action( 'wd_action_fse_before_custom_header', $content, $template, $type, $templates );
				// Custom header
				get_template_part( apply_filters( 'wd_filter_get_template_part', "templates/header-custom" ) );
				// Trigger action after the custom header to allow other modules include an additional content to the custom header
				do_action( 'wd_action_fse_after_custom_header', $content, $template, $type, $templates );
				// Get output
...
if ( ! function_exists( 'wd_gutenberg_fse_custom_header_add_side_menu' ) ) {
	add_action( 'wd_action_fse_after_custom_header', 'wd_gutenberg_fse_custom_header_add_side_menu', 10, 4 );
	/**
	 * Add a side menu to the custom header layout.
	 * 
	 * Hooks: add_action( 'wd_action_fse_after_custom_header', 'wd_gutenberg_fse_custom_header_add_side_menu', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function wd_gutenberg_fse_custom_header_add_side_menu( $content, $template, $type, $templates ) {
		// Side menu
...
if ( ! function_exists( 'wd_gutenberg_fse_custom_header_add_mobile_menu' ) ) {
	add_action( 'wd_action_fse_after_custom_header', 'wd_gutenberg_fse_custom_header_add_mobile_menu', 10, 4 );
	/**
	 * Add a mobile menu to the custom header layout.
	 * 
	 * Hooks: add_action( 'wd_action_fse_after_custom_header', 'wd_gutenberg_fse_custom_header_add_mobile_menu', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function wd_gutenberg_fse_custom_header_add_mobile_menu( $content, $template, $type, $templates ) {
		// Mobile menu
...
// Sidebar
//----------------------------------------------
if ( ! function_exists( 'wd_gutenberg_fse_add_sidebar_to_default_areas' ) ) {
	add_filter( 'default_wp_template_part_areas', 'wd_gutenberg_fse_add_sidebar_to_default_areas' );
	/**
	 * Filters the list of allowed template part area values and add 'sidebar' to this list.
	 * 
	 * Hook: add_filter( 'default_wp_template_part_areas', 'wd_gutenberg_fse_add_sidebar_to_default_areas' );
	 *
	 ...
	 */
	function wd_gutenberg_fse_add_sidebar_to_default_areas( $default_area_definitions ) {
		$default_area_definitions[] = array(
			'area'        => WD_FSE_TEMPLATE_PART_AREA_SIDEBAR,
			'label'       => esc_html__( 'Sidebar', 'wd' ),
			'description' => esc_html__( 'The Sidebar template defines a page area that typically contains a widgets.', 'wd' ),
			'icon'        => 'sidebar',
			'area_tag'    => 'div',
		);
...
if ( ! function_exists( 'wd_gutenberg_fse_modify_template_replace_sidebar' ) ) {
	add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_sidebar', 10, 4 );
	/**
	 ...
	 * 
	 * Hooks: add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_sidebar', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 ...
	 * @return string  A filtered content of the template.
	 */
	function wd_gutenberg_fse_modify_template_replace_sidebar( $content, $template, $type, $templates ) {
		...
		if ( wd_sidebar_present() ) {
			...
			if ( wd_exists_trx_addons() ) {
				...
				if ( 'custom' == $sidebar_type && ! wd_is_layouts_available() ) {
					$sidebar_type = 'default';
				}
				// Masks to search a sidebar block in the content
				$sidebar_start = apply_filters( 'wd_filter_wp_block_with_sidebar_mask_start',
												'(<!-- wp:group[\s]*{[^}]*"className":[\s]*"[^"]*sidebar[^"]*"[^>]*-->[\s]*'
												. '<div[^>]*class="[^"]*sidebar[^"]*"[^>]*>)'
												);
				$sidebar_end = apply_filters( 'wd_filter_wp_block_with_sidebar_mask_end',
												'(</div>[\s]*'
												. '<!-- /wp:group -->)'
												);
				...
				if ( 'default' == $sidebar_type ) {
					...
					if ( is_active_sidebar( $sidebar_name ) ) {
						...
						if ( preg_match( $sidebar_mask, $content ) ) {
							...
							$need_wrapper = apply_filters( 'wd_filter_wp_block_with_sidebar_wrapper_need', false );
							$content = preg_replace( '#<!-- wp:template-part[\s]*{[^}]*"slug":[\s]*"sidebar[^"]*"[^>]*/-->#U',
														( $need_wrapper
															? apply_filters(
																	'wd_filter_wp_block_with_sidebar_wrapper_start',
														...
														. ( $need_wrapper
															? apply_filters(
																	'wd_filter_wp_block_with_sidebar_wrapper_end',
																	'</div>'
																	. "\n"
																	. '<!-- /wp:group -->'
																)
				...
				// A custom sidebar (built with FSE or with any other builder)
					...
					$sidebar_style = wd_get_theme_option( "sidebar_style" );
					$sidebar_id = wd_get_custom_sidebar_id();
					...
					// Custom sidebar's layout
						...
						do_action( 'wd_action_fse_before_custom_sidebar', $content, $template, $type, $templates );
						// Custom sidebar
						do_action( 'wd_action_show_layout', $sidebar_id );
						// Trigger action after th custom sidebar to allow other modules include an additional content to the custom sidebar
						do_action( 'wd_action_fse_after_custom_sidebar', $content, $template, $type, $templates );
						// Get output
						$html = ob_get_contents();
						ob_end_clean();
						if ( ! empty( $html ) ) {
...
// Footer
//----------------------------------------------
if ( ! function_exists( 'wd_gutenberg_fse_modify_template_replace_footer' ) ) {
	add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_footer', 10, 4 );
	/**
	 * Replace a footer in the content of the current template
	 * according to the Theme Options of the current page.
	 * 
	 * Hooks: add_filter( 'wd_filter_wp_current_template_content', 'wd_gutenberg_fse_modify_template_replace_footer', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 ...
	 * @return string  A filtered content of the template.
	 */
	function wd_gutenberg_fse_modify_template_replace_footer( $content, $template, $type, $templates ) {
		$footer_type = wd_get_theme_option( 'footer_type' );
		if ( 'custom' == $footer_type ) {
			$footer_style = wd_get_theme_option( "footer_style" );
			...
			if ( strpos( $footer_style, "footer-fse-template-" ) !== false ) {
				...
				ob_start();
				do_action( 'wd_action_fse_before_custom_footer', $content, $template, $type, $templates );
				...
				ob_start();
				do_action( 'wd_action_fse_after_custom_footer', $content, $template, $type, $templates );
			...
			// Custom footer's layout
				...
				do_action( 'wd_action_fse_before_custom_footer', $content, $template, $type, $templates );
				// Custom footer
				get_template_part( apply_filters( 'wd_filter_get_template_part', "templates/footer-custom" ) );
...
//------------------------------------------------------
if ( ! function_exists( 'wd_gutenberg_fse_replace_featured_image_renderer' ) ) {
	add_filter( 'block_type_metadata_settings', 'wd_gutenberg_fse_replace_featured_image_renderer', 10, 2 );
	/**
	 ...
	 */
	function wd_gutenberg_fse_replace_featured_image_renderer( $settings = array(), $metadata = array() ) {
		if ( ! empty( $settings['render_callback'] ) && $settings['render_callback'] == 'render_block_core_post_featured_image' ) {
			$settings['render_callback'] = 'wd_gutenberg_fse_featured_image_renderer';
		}
		return $settings;
	}
}

if ( ! function_exists( 'wd_gutenberg_fse_featured_image_renderer' ) ) {
	/**
	 ...
	 */
	function wd_gutenberg_fse_featured_image_renderer( $attributes, $content, $block ) {
		...
		$wd_hover      = wd_get_theme_option( 'image_hover' );
		$wd_components = wd_array_get_keys_by_value( wd_get_theme_option( 'meta_parts' ) );
		...
		wd_show_post_featured( apply_filters( 'wd_filter_args_featured',
			array(
				...
				'singular'   => wd_is_singular(),
				'thumb_size' => wd_get_thumb_size( strpos( wd_get_theme_option( 'body_style' ), 'full' ) !== false
									? 'full'
									: ( $wd_expanded || wd_is_singular()
										? 'huge' 
										: 'big' 
										)
									),
