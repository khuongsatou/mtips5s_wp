...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			wd_create_observer( 'wd-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				wd_gutenberg_editor_init();
			} );
		} else {
		 	wd_gutenberg_editor_init();
		}
	...
	function wd_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'wd_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'wd_inited' ) ) {
	...
	// Init on page load
	function wd_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = wd_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + WD_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + WD_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in WD_STORAGE['override_classes'] ) {
				$obj.addClass( WD_STORAGE['override_classes'][i].replace( '%s', WD_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( wd_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && WD_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'wd_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			wd_remove_observer( 'wd-check-editor-styles-wrapper' );
			wd_create_observer( 'wd-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'wd_inited' );
	}
} );
