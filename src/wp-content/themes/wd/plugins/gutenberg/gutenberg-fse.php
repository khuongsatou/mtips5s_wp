...
if ( ! defined( 'WD_FSE_TEMPLATE_PART_PT' ) ) define( 'WD_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

if ( ! defined( 'WD_FSE_TEMPLATE_PART_AREA_HEADER' ) )  define( 'WD_FSE_TEMPLATE_PART_AREA_HEADER',  defined( 'WP_TEMPLATE_PART_AREA_HEADER' )  ? WP_TEMPLATE_PART_AREA_HEADER  : 'header' );
if ( ! defined( 'WD_FSE_TEMPLATE_PART_AREA_FOOTER' ) )  define( 'WD_FSE_TEMPLATE_PART_AREA_FOOTER',  defined( 'WP_TEMPLATE_PART_AREA_FOOTER' )  ? WP_TEMPLATE_PART_AREA_FOOTER  : 'footer' );
if ( ! defined( 'WD_FSE_TEMPLATE_PART_AREA_SIDEBAR' ) ) define( 'WD_FSE_TEMPLATE_PART_AREA_SIDEBAR', defined( 'WP_TEMPLATE_PART_AREA_SIDEBAR' ) ? WP_TEMPLATE_PART_AREA_SIDEBAR : 'sidebar' );
if ( ! defined( 'WD_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) define( 'WD_FSE_TEMPLATE_PART_AREA_UNCATEGORIZED', defined( 'WP_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ? WP_TEMPLATE_PART_AREA_UNCATEGORIZED : 'uncategorized' );

// Load additional files with FSE support
require_once wd_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once wd_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'wd_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'wd_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function wd_gutenberg_fse_loader() {
		// 
		if ( wd_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& wd_is_on( wd_get_theme_option( 'front_page_enabled', false ) )
			&& wd_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'wd_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function wd_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'wd_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function wd_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'wd_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function wd_exists_gutenberg_fse() {
		return wd_exists_gutenberg() && ( wd_gutenberg_is_fse_wp_support() || wd_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'wd_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function wd_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( wd_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( wd_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( WD_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( WD_THEME_DIR != WD_CHILD_DIR && is_readable( WD_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( WD_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( WD_THEME_DIR != WD_CHILD_DIR && is_readable( WD_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'wd_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function wd_gutenberg_is_fse_enabled() {
		return wd_exists_gutenberg_fse() && wd_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'wd_gutenberg_fse_skin_updated' ) ) {
	add_action( 'wd_action_theme_updated', 'wd_gutenberg_fse_skin_updated' );
	add_action( 'wd_action_skin_updated', 'wd_gutenberg_fse_skin_updated', 10, 1 );
	/**
	 * Copy FSE-required folders from an updated skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'wd_action_skin_updated', 'wd_gutenberg_fse_skin_updated', 10, 1 );
	 */
	function wd_gutenberg_fse_skin_updated( $skin = '' ) {
		// Get an active skin name if empty
		if ( current_action() == 'wd_action_theme_updated' && empty( $skin ) ) {
			$skin = wd_skins_get_active_skin_name();
		}
		if ( wd_skins_get_current_skin_name() == $skin ) {
			wd_gutenberg_fse_copy_from_skin( $skin );
			wd_gutenberg_fse_update_theme_json( $skin );
		}
	}
}

if ( ! function_exists( 'wd_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'wd_action_skin_switched', 'wd_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin's subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'wd_action_skin_switched', 'wd_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function wd_gutenberg_fse_copy_from_skin( $new_skin, $old_skin = '' ) {
		$theme_templates_dir = wd_prepare_path( WD_THEME_DIR . ( wd_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = wd_prepare_path( WD_THEME_DIR . ( wd_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = wd_prepare_path( WD_THEME_DIR . 'theme.json' );
		$skin_templates_dir = wd_prepare_path( WD_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( wd_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = wd_prepare_path( WD_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( wd_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			wd_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			wd_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			wd_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			wd_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			wd_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'wd_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'wd_action_skin_switched', 'wd_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'wd_action_save_options', 'wd_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'wd_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'wd_action_skin_switched', 'wd_gutenberg_fse_update_theme_json', 30, 2 );
	 * 
	 *        add_action( 'wd_action_save_options', 'wd_gutenberg_fse_update_theme_json', 30 );
	 * 
	 *        add_action( 'trx_addons_action_save_options', 'wd_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'wd_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function wd_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// Get an active skin name if empty
		if ( in_array( current_action(), array( 'wd_action_save_options', 'trx_addons_action_save_options' ) ) && empty( $new_skin ) ) {
			$new_skin = wd_skins_get_active_skin_name();
		}
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) && is_dir( WD_THEME_DIR . wd_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = wd_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				wd_fpc( wd_prepare_path( WD_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'wd_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'wd_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function wd_gutenberg_fse_theme_json_data() {
		...
		$data = array(
					...
					"customTemplates" => array(
						array(
							"name"      => "blog",
							"title"     => esc_html__( "Blog archive", 'wd' ),
							"postTypes" => array(
								"page"
							)
						)
					...
					"templateParts" => array(
						array(
							"name"  => "header",
							"title" => esc_html__( "Header for FSE", 'wd' ),
							"area"  => WD_FSE_TEMPLATE_PART_AREA_HEADER,
						),
						array(
							"name"  => "sidebar",
							"title" => esc_html__( "Sidebar for FSE", 'wd' ),
							"area"  => WD_FSE_TEMPLATE_PART_AREA_SIDEBAR,
						),
						array(
							"name"  => "content-404",
							"title" => esc_html__( "Page 404 content", 'wd' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-archive",
							"title" => esc_html__( "No posts found", 'wd' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "none-search",
							"title" => esc_html__( "Nothing matched search", 'wd' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-standard",
							"title" => esc_html__( "Blog post item: Standard", 'wd' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-post-header",
							"title" => esc_html__( "Blog post header", 'wd' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "blog-pagination",
							"title" => esc_html__( "Blog pagination", 'wd' ),
							"area"  => WP_TEMPLATE_PART_AREA_UNCATEGORIZED,
						),
						array(
							"name"  => "footer",
							"title" => esc_html__( "Footer for FSE", 'wd' ),
...
*/
		...
		$data['settings']['color']['duotone'][] = array(
			...
			"slug"   => 'bg-and-text',
			"name"   => esc_html__( 'Background and text color', 'wd' )
		);
		$data['settings']['color']['duotone'][] = array(
			"colors" => array( $scheme['bg_color'], $scheme['text_dark'] ),
			"slug"   => 'bg-and-dark',
			"name"   => esc_html__( 'Background and dark color', 'wd' )
		);
		$data['settings']['color']['duotone'][] = array(
			"colors" => array( $scheme['bg_color'], $scheme['text_link'] ),
			"slug"   => 'bg-and-link',
			"name"   => esc_html__( 'Background and link color', 'wd' )
		);
		...
		$data['settings']['color']['gradients'][] = array(
			...
			"name"     => esc_html__( 'Vertical from link color to hover color', 'wd' ),
		);
		$data['settings']['color']['gradients'][] = array(
			...
			"name"     => esc_html__( 'Diagonal from link color to hover color', 'wd' ),
		);
/*
		// Add fonts
...
*/
		...
		return apply_filters( 'wd_filter_gutenberg_fse_theme_json_data', $data );
	}
}
