...
class NavMenuWidget extends BaseWidget {
	...
	private function register_content_settings_controls() {
		...
		$templates = ! $is_edit_mode ? array() : Utils::get_elementor_page_list();	//trx_addons_get_list_elementor_templates();
	...
	private function register_content_menu_controls() {
		...
		// 		'type'               => Controls_Manager::SELECT2,
		// 		'frontend_available' => true,
		// 		'options'            => Utils::get_all_breakpoints(),
		// 		'multiple'           => true,
		// 		'label_block'        => true,
		// 		'render_type'        => 'template',
				...
				if ( 'link' === $item['menu_content_type'] ) {
					...
					if ( 'custom_content' === $item['menu_content_type'] ) {
						...
						if ( ! empty( $item['submenu_item'] ) ) {
							...
							$html_output .= Utils::get_template_content( $temp_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
